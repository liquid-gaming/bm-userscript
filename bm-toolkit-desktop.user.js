...
const selectors = {
    root: "#root",
    rconLayout: "#RCONLayout",
    rconContainer: "#RCONContainer",

    serverLayout: "#RCONServerLayout",
    serverCommands: "#server-commands",
    serverCommandButtons: '#server-commands button[data-command]',

    playerPage: "#RCONPlayerPage",
    playerName: "#RCONPlayerPage h1",
    playerLinks: 'a[href^="/rcon/players/"]',

    activity: '[data-testid="activity"]',
    activityItems: '[data-testid="activity-item"]',
    timestamps: "time[datetime], [datetime]",

    steamId: '[title*="765"]',
    eosId: '[title*="0002"]',
    ovhLogo: "#poweredbyovh"
};
...
function pageReady() {
    return (
        document.querySelector(selectors.rconLayout) ||
        document.querySelector(selectors.rconContainer) ||
...
function isPlayerPage() {
    return (
        document.querySelector(selectors.playerPage) ||
        /^\/rcon\/players\/\d+/.test(location.pathname)
    );
}

function isServerPage() {
    return (
        document.querySelector(selectors.serverLayout) ||
        /^\/rcon\/servers\/\d+/.test(location.pathname)
    );
}
...
function cornerButtons() {
    if (document.getElementById("liq-corner-buttons")) return;
    ...
    buttonContainer.id = "liq-corner-buttons";
    ...
    buttons.forEach(({ id, label, url, backgroundColor, fontSize }) => {
...
function applyTimeStamps() {
    document.querySelectorAll(selectors.timestamps).forEach(element => {
...
function applyColor(elements, phrases, color) {
...
function adminApplyColor(elements, phrases, color) {
    elements.forEach(el => {
        phrases.forEach(phrase => {
            ...
            const regex = new RegExp(`(\\b${safePhrase}\\b)|(\\b『LiQ』 ?${safePhrase}\\b)`, "i");
...
function getActivityElements() {
    ...
    // Fallback: only rows that look like activity/admin log entries.
    // Avoid scanning the entire page because that colors Notes/Admin Log/form labels.
    return [...document.querySelectorAll("li, tr, article, [class]")]
        .filter(el => {
            const text = el.textContent?.trim() || "";

            const looksLikeLogEntry =
                text.includes(" was kicked ") ||
                text.includes(" was warned ") ||
                text.includes(" was banned ") ||
                text.includes(" joined the server") ||
                text.includes(" left the server") ||
                text.includes(" team killed ") ||
                text.includes(" requested ") ||
                text.includes(" changed the map ") ||
                text.includes(" set the next map ") ||
                text.includes(" restarted the match") ||
                text.includes("Trigger added flag");

            return looksLikeLogEntry && text.length < 800;
        });
}
...
function logColoring() {
    ...
    applyColor(activityItems, sets.grayedOut, colors.cGrayed);
    applyColor(activityItems, sets.joinedServer, colors.cJoined);
    applyColor(activityItems, sets.leftServer, colors.cLeftServer);
    applyColor(activityItems, sets.teamBluefor, colors.cTeamBluefor);
    applyColor(activityItems, sets.teamOpfor, colors.cTeamOpfor);
    ...
    [...document.querySelectorAll("span, div, strong")]
        .filter(el => el.children.length === 0 && el.textContent.trim() === "Admin")
        .forEach(el => {
            el.style.color = colors.cbmAdmin;
        });

    // Only note/flag icons by aria/title, not Notes section text.
    document.querySelectorAll('i, svg, span.glyphicon').forEach(element => {
    const label = (
    ...
    if (
        (label.includes("note") || label.includes("flag")) &&
...
function createCopyButtons() {
    ...
    copyButton.addEventListener("click", () => {
        ...
        const pName = document.querySelector(selectors.playerName)?.innerText || "NAME MISSING?";
    ...
    openURLButton.addEventListener("click", () => {
        ...
        if (pSteamID && pSteamID !== "SteamID MISSING?") {
            window.open(`https://communitybanlist.com/search/${pSteamID}`, "_blank");
        } else {
...
function playerPageTools() {
    if (isPlayerPage()) {
        ensureElementExists("copy-button", createCopyButtons);
...
async function runDataFetching() {
    ...
    const pSteamID = getInnerTextByTitle("765", "SteamID MISSING?");
    if (!pSteamID || pSteamID === "SteamID MISSING?") return;
...
async function fetchSteamUserData(steamID) {
    ...
    while (attempt < maxRetries) {
        try {
            ...
            console.error(`CBL fetch attempt ${attempt} failed:`, error);
...
function displayUserData(riskRating, activeBansCount, expiredBansCount) {
    ...
    let riskColor = "white";
    let displayRisk = riskRating;

    if (riskRating >= 1 && riskRating <= 5) {
        riskColor = "orange";
        displayRisk = `${riskRating}/10`;
...
function colorServerCommandMenu() {
    document.querySelectorAll(selectors.serverCommandButtons).forEach(button => {
        const text = button.textContent.trim();

        if (text.includes("Warn")) button.style.color = "lime";
        if (text.includes("Kick")) button.style.color = "orange";
        if (text.includes("Ban")) button.style.color = "red";
        if (text.includes("Force Team Change")) button.style.color = "#db4dff";
        if (text.includes("Remove Player from Squad")) button.style.color = "#804d00";
        if (text.includes("Action - Reset Squad Name")) button.style.color = "gold";

        if (text.includes("Set Next Layer") || text.includes("Next Layer")) {
            button.style.color = "lime";
            button.style.fontSize = "16pt";
        }

        if (text.includes("Change Layer")) {
            button.style.color = "red";
            button.style.fontWeight = "bold";
        }

        if (text.includes("Squad List")) {
...
function colorDialogMenus() {
    const dialogItems = document.querySelectorAll(
        '.modal-title, [role="dialog"] button, [role="dialog"] a, .modal button, .modal a'
    );

    dialogItems.forEach(el => {
        const text = el.textContent.trim();
        ...
        if (text.includes("Force Team Change")) {
            el.style.color = "#db4dff";
        }

        if (text.includes("Remove Player from Squad")) {
            el.style.color = "#804d00";
        }

        if (text.includes("Action - Reset Squad Name")) {
            el.style.color = "gold";
        }
    });
}
...
function runCode() {
    console.log("BattleMetrics toolkit started.");
