...
// @run-at document-end
// ==/UserScript==
const version="12.17";const updateRate=150;const colors={cTeamBluefor:"#4eacff",cTeamOpfor:"#d0b1ff",cTeamIndepend:"#fd6aff",cAdminName:"#00fff7",cbmAdmin:"#58ff47",cModAction:"#ff3333",cAdminAction:"#37ff00",cTeamKilled:"#ffcc00",cLeftServer:"#d9a6a6",cJoined:"#919191",cGrayed:"#919191",cTracked:"#FF931A",cNoteColorIcon:"#f5ccff"};const selectors={root:"#root",rconLayout:"#RCONLayout",rconContainer:"#RCONContainer",serverLayout:"#RCONServerLayout",serverCommands:"#server-commands",serverCommandButtons:"#server-commands button[data-command]",playerPage:"#RCONPlayerPage",playerName:"#RCONPlayerPage h1",playerLinks:'a[href^="/rcon/players/"]',activity:'[data-testid="activity"]',activityItems:'[data-testid="activity-item"]',timestamps:"time[datetime], [datetime]",steamId:'[title*="765"]',eosId:'[title*="0002"]',ovhLogo:"#poweredbyovh"};const sets={teamKilled:new Set(["team killed"]),grayedOut:new Set(["AFK - Thanks for playing!","Please get a Squad Leader kit within 8 mins","Final warning: Get Squad Leader kit within 5m","SEEDING WHITELIST ACTIVE! Thanks for helping seed the server!","You will be kicked in 2 minutes if you are still not in a squad","To switch teams, please run the","Check your seeding reward status via","Trigger added flag LiQ Seeder",'Congrats to our seeding rewards giveaway winners! Type "!seedrewards" in chat for more info.']),trackedTriggers:new Set(["[SL Kit]"]),leftServer:new Set(["left the server"]),joinedServer:new Set(["joined the server"]),actionList:new Set(["was warned","was kicked","was banned","edited BattleMetrics Ban","added BattleMetrics Ban","deleted BattleMetrics Ban","Trigger added flag Previously banned"]),adminList:new Set(["Amen774","ANGEL_42","AWMDemons","basa/doc","bloodmoon529","Capt-Crossbones","Chillz","davisfoxes","dontfakeit","Ethan","ETXBONES","Exploits","got2bhockey","budge","chaot3ch","GRIM_CURLY","HellHound6396","hellsaber","iCampHard","Jonboy","Kibz","Kyle","Ligmas","Lucrass Kelvac","MODERNMEGA","Moses","outlast","POM_Hephoof","redneck","Shaka","spankinkoala","temper","Terminator","Thelifesteeler","Valkyrie","Wasted","white knife","wjli13125","wwrp450","WobbleBob29","xplay0321","ZB"]),teamBluefor:new Set(["Australian Defence Force","British Armed Forces","Canadian Armed Forces","United States Army","United States Marine Corps","Turkish Land Forces","Armed Forces of Ukraine"]),teamOpfor:new Set(["Russian Ground Forces","Middle Eastern Alliance","Middle Eastern Insurgents","Insurgent Forces","People's Liberation Army","Russian Airborne Forces","PLA Navy Marine Corps","PLA Amphibious Ground Forces","Western Private Military Contractors","Ground Forces of Iran"]),teamIndepend:new Set(["Western Private Military Contractors"]),adminTerms:new Set(["to the other team.",") was disbanded b","requested a list of squads.","set the next map to","changed the map to","requested the next map.",") forced","AdminRenameSquad","(Global)","executed Player Action Action","requested the current map.","restarted the match.","Squad disband - SL","was removed from their squad by Trigger.","requested layer list.","was removed from their squad by","unpossessed the admin camera","possessed the admin camera"])};let isFetching=false;function sleep(ms){return new Promise(resolve=>setTimeout(resolve,ms))}function injectStyle(cssText){const style=document.createElement("style");style.textContent=cssText;document.head.appendChild(style)}function pageReady(){return document.querySelector(selectors.rconLayout)||document.querySelector(selectors.rconContainer)||document.querySelector(".navbar-brand")}function isPlayerPage(){return document.querySelector(selectors.playerPage)||/^\/rcon\/players\/\d+/.test(location.pathname)}function isServerPage(){return document.querySelector(selectors.serverLayout)||/^\/rcon\/servers\/\d+/.test(location.pathname)}function addBaseStyles(){injectStyle(`
        ${selectors.ovhLogo} {
